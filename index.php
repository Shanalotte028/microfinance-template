        ...
        <table id="datatablesSimple">
            <thead>
                <tr>
                    ...
                    <th>First Name</th>
                    <th>Last Name</th>
                    <th>Age</th> <!-- Added Email Column -->
                    <th>Email</th> <!-- Added Age Column -->
                    <th>Address</th>
            ...
            <tbody>
                ...
                if (isset($_POST['action']) && isset($_POST['id'])) {
                    ...
                    if ($action == 'approve') {
                        ...
                        $update_query = "UPDATE images_coe_birthc SET status = 'Approved', date_status_updated = NOW() WHERE id = $id";
                    } elseif ($action == 'decline') {
                        // Update the status to 'Declined' and set the date_status_updated
                        $update_query = "UPDATE images_coe_birthc SET status = 'Declined', date_status_updated = NOW() WHERE id = $id";
                    } elseif ($action == 'remove') {
                        // Fetch the file paths from the database
                        $query = "SELECT coe, birthc FROM images_coe_birthc WHERE id = $id";
                        $result = mysqli_query($conn, $query);
                        $row = mysqli_fetch_assoc($result);

                        // Delete the files from the server
                        $coe_file = 'images-coe-birthc/' . $row['coe'];
                        $birthc_file = 'images-coe-birthc/' . $row['birthc'];

                        if (file_exists($coe_file)) {
                            unlink($coe_file);
                        }

                        if (file_exists($birthc_file)) {
                            unlink($birthc_file);
                        }

                        // Delete the record from the database
                        $delete_query = "DELETE FROM images_coe_birthc WHERE id = $id";
                    ...
                    // Execute the update query for approval or decline
                ...
                ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['Age']); ?></td> <!-- Display Email -->
                            <td><?php echo htmlspecialchars($row['email']); ?></td> <!-- Display Age -->
                            <td><?php echo htmlspecialchars($row['address']); ?></td>
                            <td><img src="images-coe-birthc/<?php echo htmlspecialchars($row['coe']); ?>" alt="COE" style="width: 100px;"></td>
                            <td><img src="images-coe-birthc/<?php echo htmlspecialchars($row['birthc']); ?>" alt="Birth Certificate" style="width: 100px;"></td>
